...
function Initiate_Push_Send(
  ...
  $query_push_json = pg_query(
    $connection,
    "SELECT push_json FROM log_in_sessions WHERE profile_code = '$profile_code' ORDER BY last_seen DESC;",
  );
  while ($data_push_json = pg_fetch_assoc($query_push_json)) {
    ...
    if ($push_json !== "") {
      ...
      shell_exec(
        $php_path .
          " ../../Server/Push_Sender.php " .
          $push_json .
          " " .
          $data_json .
          " " .
          $background_null_device,
      );
    }
  }
}
