...
function Retrieve_Log_In_Sessions()
{
  ...
  $query_log_in_sessions = pg_query(
    $connection,
    "SELECT serial, token, platform, agent, last_ip_address, last_seen FROM log_in_sessions WHERE profile_code = '$my_profile_code' ORDER BY last_seen DESC;",
  );
  ...
  while ($data_log_in_sessions = pg_fetch_assoc($query_log_in_sessions)) {
    ...
    if ($token == "" || $token == null) {
      ...
      $query_check_current_session = pg_query(
        $connection,
        "SELECT serial FROM log_in_sessions WHERE serial = '$serial' AND profile_code = '$my_profile_code' AND token = '$token';",
      );
