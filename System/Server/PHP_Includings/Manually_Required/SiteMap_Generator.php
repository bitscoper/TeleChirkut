...
function Generate_SiteMap()
{
  ...
  $query_profile_codes = pg_query(
    $connection,
    "SELECT profile_code FROM users ORDER BY profile_code ASC;",
  );
  while ($data_profile_code = pg_fetch_assoc($query_profile_codes)) {
    $url_element = $sitemap->addChild("url");
    $url_element->addChild(
      "loc",
      "https://telechirkut.bitscoper.dev/#profile=" .
        $data_profile_code["profile_code"],
    );
  }
