...
function Receive_Post($text)
{
  ...
  if (($image_available && $valid_image) || !$image_available) {
    ...
    if ($image_available && $valid_image) {
      $query = pg_query(
        $connection,
        "INSERT INTO feedline(profile_code, image, text, at) VALUES('$my_profile_code', true, '$text', '$at') RETURNING serial;",
      );
    } else {
      $query = pg_query(
        $connection,
        "INSERT INTO feedline(profile_code, image, text, at) VALUES('$my_profile_code', NULL, '$text', '$at') RETURNING serial;",
      );
    }
  ...
  if ($image_available && $valid_image) {
    Process_Image(
      ...
      1024,
      -1,
      $user_uploads_path . "Post_Images/FeedLine_" . $serial . ".webp",
    );
  }
