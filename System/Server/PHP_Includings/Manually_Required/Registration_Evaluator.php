...
function Evaluate_Registration()
{
  ...
  if (isset($_COOKIE["TeleChirkut_1"]) && isset($_COOKIE["TeleChirkut_2"])) {
    ...
    $query_authentication = pg_query(
      $connection,
      "SELECT profile_code FROM log_in_sessions WHERE profile_code = '$my_profile_code' AND token = '$token';",
    );

    if (
      Check_User($my_profile_code) &&
      pg_num_rows($query_authentication) == 1
    ) {
      $return_value = true;
    } else {
