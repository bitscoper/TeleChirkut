...
function Change_PassWord($current_password, $new_password, $retyped_password)
{
  ...
  require_once $includings_path .
    "Manually_Required/PassWord_Hash_Retriever.php";

  if (
    password_verify($current_password, Retrieve_PassWord_Hash($my_profile_code))
  ) {
    if ($current_password !== $new_password) {
      if ($new_password == $retyped_password) {
        if (strlen($new_password) >= 11) {
          ...
          pg_query(
            $connection,
            "UPDATE users SET password_hash = '$new_password_hash' WHERE profile_code = '$my_profile_code';",
          );
          ...
          pg_query(
            $connection,
            "UPDATE log_in_sessions SET token = '', push_json = '' WHERE profile_code = '$my_profile_code' AND NOT token = '$token';",
          );
    ...
    $body =
      "There has been a failed attempt to change your password from IP Address " .
      $ip_address;
    Initiate_Send_EMail(
      $my_profile_code,
    ...
    Initiate_Push_Send(
      $my_profile_code,
      "Security",
      "There has been a failed attempt to change your password.",
      "",
      "",
    );
  }
