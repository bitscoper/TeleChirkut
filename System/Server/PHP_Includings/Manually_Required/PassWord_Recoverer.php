    ...
    if ($new_password == $retyped_password) {
      if (strlen($new_password) >= 11) {
        if (strlen($authentication_code) >= 8) {
          ...
          $query_user = pg_query(
            $connection,
            "SELECT last_recovery_verification_code, last_recovery_authentication_code FROM users WHERE profile_code = '$profile_code';",
          );

          while ($data_user = pg_fetch_assoc($query_user)) {
            $retrieved_verification_code =
              $data_user["last_recovery_verification_code"];
            $retrieved_authentication_code =
              $data_user["last_recovery_authentication_code"];
          }
          ...
          if ($retrieved_verification_code == $verification_code) {
            if ($retrieved_authentication_code == $authentication_code) {
              ...
              require_once $includings_path .
                "Manually_Required/Client_IP_Address_Retriever.php";
              ...
              pg_query(
                $connection,
                "UPDATE users SET password_hash = '$new_password_hash', last_recovery_verification_code = '', last_recovery_authentication_code = 0 WHERE profile_code = '$profile_code';",
              );

              pg_query(
                $connection,
                "UPDATE log_in_sessions SET token = '', push_json = '' WHERE profile_code = '$profile_code';",
              );
              ...
              $body =
                "Your password has been changed from IP Address " .
                $ip_address .
                " through recovery option.";
