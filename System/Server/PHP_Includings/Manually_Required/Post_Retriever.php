...
function Retrieve_Post($serial)
{
  ...
  $query_post = pg_query(
    $connection,
    "SELECT profile_code, text, image, at FROM feedline WHERE serial = '$serial';",
  );

  while ($data_post = pg_fetch_assoc($query_post)) {
    $profile_code = $data_post["profile_code"];

    $query_full_name_verification = pg_query(
      $connection,
      "SELECT full_name, verification FROM users WHERE profile_code = '$profile_code';",
    );

    while (
      $data_full_name_verification = pg_fetch_assoc(
        $query_full_name_verification,
      )
    ) {
    ...
    $query_reactions = pg_query(
      $connection,
      "SELECT type FROM post_reactions WHERE feedline_serial = '$serial';",
    );
    ...
    if ($profile_code !== $my_profile_code) {
      $query_reacted = pg_query(
        $connection,
        "SELECT type FROM post_reactions WHERE feedline_serial = '$serial' AND from_profile_code = '$my_profile_code';",
      );
