...
if ($TeleChirkut) {
  ...
  require_once $includings_path .
    "Manually_Required/Shell_Execution_Variables.php";
  ...
  header(
    "Content-Security-Policy: script-src 'self' 'unsafe-eval' 'nonce-" .
      $javascript_nonce .
      "'; script-src-elem 'self' 'unsafe-eval'; script-src-attr 'self' 'unsafe-eval'; img-src 'self' data:; font-src 'self'; connect-src 'self'; media-src 'self'; object-src 'none'; child-src 'none'; frame-src 'none'; worker-src 'self'; frame-ancestors 'none'; form-action 'self'; upgrade-insecure-requests; block-all-mixed-content; sandbox allow-forms allow-same-origin allow-scripts allow-top-navigation allow-popups; base-uri 'self'; manifest-src 'self'",
  );

  require_once $includings_path .
    "Manually_Required/Paired_Cookies_CareTaker.php";
  ...
  require_once $includings_path .
    "Manually_Required/Registration_Evaluator.php";
  ...
  if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["sitemap"])) {
    ...
    if (intval($time)) {
      require_once $includings_path .
        "Manually_Required/JavaScript_Bundler.php";
    ...
    if ($public == "registration") {
      if (!$registration) {
        require_once $includings_path .
          "Manually_Required/Registration_Page_Generator.php";
    ...
    isset($_GET["image_message"])
  ) {
    $serial = Sanitize_String($_GET["image_message"]);

    require_once $includings_path .
      "Manually_Required/Image_Message_Retriever.php";
    ...
    require_once $includings_path .
      "Manually_Required/Post_Image_Retriever.php";
    ...
    require_once $includings_path .
      "Manually_Required/Error_Page_Generator.php";

    $error_code = Sanitize_String($_GET["error"]);

    if ($error_code == 403) {
      http_response_code(403);
      echo Generate_Error_Page(
        $error_code,
        "You are not permitted to request this!",
      );
    ...
    $_SERVER["REQUEST_METHOD"] == "GET" &&
    isset($_GET["notify_unseen_counts"])
  ) {
    require_once $includings_path .
      "Manually_Required/Unseen_Counts_Notifier.php";
    ...
    $_SERVER["REQUEST_METHOD"] == "GET" &&
    isset($_GET["reset_test_user_password"])
  ) {
    require_once $includings_path .
      "Manually_Required/Test_User_PassWord_Resetter.php";
    ...
    isset($_POST["recovery_email_address"]) &&
    is_uploaded_file($_FILES["face_logo"]["tmp_name"])
  ) {
    ...
    require_once $includings_path .
      "Manually_Required/Registered_POST_Request_Handler.php";

    echo Handle_Registered_POST_Requests();
  } elseif (
    $_SERVER["REQUEST_METHOD"] == "POST" &&
    isset($_POST["client_caches"])
  ) {
    require_once $includings_path .
      "Manually_Required/Client_Caches_Lister.php";
    ...
    if ($type == "PassWord_Recovery_1" && isset($_POST["profile_code"])) {
      ...
      require_once $includings_path .
        "Manually_Required/PassWord_Recovery_Initiator.php";

      header("Content-Type: application/json");
      echo Initiate_PassWord_Recovery($profile_code);
    } elseif (
      $type == "PassWord_Recovery_2" &&
      isset($_POST["profile_code"]) &&
      isset($_POST["verification_code"]) &&
      isset($_POST["authentication_code"]) &&
      ...
      require_once $includings_path .
        "Manually_Required/PassWord_Recoverer.php";
    ...
    if (!$registration) {
      ...
      require_once $includings_path .
        "Manually_Required/Client_Page_Generator.php";
